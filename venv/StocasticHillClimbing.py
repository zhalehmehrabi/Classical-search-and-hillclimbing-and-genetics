...
def stocasticHillClimbing(colorHill, graphHill, nodeNum):
    queue = []
    extendSize = 0
    visitSize = 1
    ...
    while 1:
        ...
        # that is the score of starting state and I save a copy of it to use when i want to pop from queue
        ...
        for x in range(1, nodeNum + 1):
            ...
            actionForward(colorHill, x)
        # here i filter the adjacentes that their score is lower than score of starting state of this step
        if sys.getsizeof(queue) > storage:
            storage = sys.getsizeof(queue)
        queue = [(x, y) for x, y in queue if y <= scoreHill]
        # now we find the best move inside of all action that we could take using a heap sort
        heapSort(queue)
        # now i pop one of the answers randomly
        bestColor, bestScore = queue.pop(random.randint(0, queue.__len__() - 1))
        extendSize += 1
        # if we can not find a better adjacent,we find one answer,it can be local maximal
        if bestColor == listHill:
            print("colors : ")
            print(bestColor)
...
if __name__ == "__main__":
    ...
    extend, visit, storage = stocasticHillClimbing(color, graph, numberOfNodes)
