...
def hillClimbingSimple(colorHill, graphHill, nodeNum):
    ...
    while 1:
        if sys.getsizeof(queue) + sys.getsizeof(answerQueue) > storage:
            storage = sys.getsizeof(queue) + sys.getsizeof(answerQueue)
        # this is the times that algorithm start again
        if answerQueue.__len__() > 1000:
            heapSort(answerQueue)
            bestFindcolor, bestFindScore = answerQueue.pop(0)
            if bestFindScore == 0:
                print("colors : ")
                print(bestFindcolor)
                print("score for this colors : " + str(bestFindScore))
        ...
        for x in range(1, nodeNum + 1):
            ...
            if score > tempForward:
                score = tempForward
            # the first will undo the action forward of the line 89
        ...
        if bestColor == listHill or (((listHill, scoreHill) in queue) and scoreHill == bestScore):
            if bestScore == 0:
                ...
                answerQueue.append((bestColor, bestScore))
                visitSize += 1
                randomStarter(colorHill)
                continue
        else:
...
# here is where the state is generate randomly
def randomStarter(colorRandomStarter):
    for u in colorRandomStarter:
        colorRandomStarter[u][0] = random.randint(1, numberOfColors)


if __name__ == "__main__":
    numberOfNodes = graph.__len__()
    # print(evaluation(color, graph))
